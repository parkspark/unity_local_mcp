...
- This applies in BOTH directions. If the handler sits on the coin instead of the player, still identify the other side by component — `if (other.GetComponent<PlayerMovement>() != null)` — never `other.CompareTag("Player")`. **No tool in this session can set a GameObject tag**, so every object stays `Untagged` and a CompareTag branch is false forever: the pickup silently never fires and the run still reports the objects as present.
- `OnTriggerEnter` only fires when one of the two colliders IS a trigger. Writing the handler is not enough: set `isTrigger = true` on the PICKUP's collider (`unity_set_component_property` on its SphereCollider/BoxCollider). Measured across recorded runs, 6 of 13 contact requests wrote the handler and never made anything a trigger — the branch is dead and the item behaves as a wall the player bumps into. Never make the PLAYER's collider a trigger; it then falls through the floor.
- A trigger collider does not collide with the ground, so an item that has BOTH `isTrigger` and a non-kinematic Rigidbody falls out of the world (measured: y 1.0 → −45 within one measurement) and the player reaches an empty spot. Give pickups NO Rigidbody at all — the player's Rigidbody is what makes `OnTriggerEnter` fire. If one is unavoidable, set `isKinematic = true`.
...
- This project uses the NEW Input System ONLY. NEVER use the legacy UnityEngine.Input API (Input.GetAxis, Input.GetButtonDown, Input.GetKey...) — it throws InvalidOperationException at runtime. Instead `using UnityEngine.InputSystem;` and read `Keyboard.current` / `Mouse.current` / `Gamepad.current`, e.g. `keyboard.aKey.isPressed`. Always null-check `Keyboard.current` first. For a short jump tap, detect the rising edge yourself: `bool pressed = keyboard.spaceKey.isPressed; if (pressed && !jumpHeld && isGrounded) jumpRequested = true; jumpHeld = pressed;` in Update, then apply exactly one `AddForce(..., ForceMode.Impulse)` in FixedUpdate and set `jumpRequested = false`. Do NOT rely on `wasPressedThisFrame` (its queued edge can be gone before gameplay Update observes it) and do NOT latch on `isPressed` alone (it re-arms every frame the key is held, so FixedUpdate stacks one impulse per physics step and the player launches several times too high). For simple keyboard gameplay, read `Keyboard.current` directly and do NOT create or add PlayerInput, InputActionAsset, generated controls, or callback handler components; direct device reads need none of them and unwired action components swallow the task budget without delivering input.
