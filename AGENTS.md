...
현재 v1.11.31. 이 저장소에서 작업하는 모든 에이전트는 아래 규칙을 따른다.
...
4. **`docs/STATUS.md` 갱신** — 해소한 항목은 F로 옮기고, 새로 연 항목은 추가한다
...
### 3.1 버전 문서에 "남은 과제"를 적지 않는다

버전 문서는 **그 시점의 불변 증거 기록**이고, 남은 과제는 **덮어써야 하는 현재
상태**다. 두 역할을 한 파일에 겹쳐 놓았더니 문서마다 "이전 문서의 항목은 그대로
남았다"는 이월 사슬이 생겼다. v1.11.17 시점에 15개 문서가 `남은 과제` 섹션을 갖고,
최근 4개에 나열된 21개 항목의 실제 고유 항목은 12개였다. 지금 무엇이 열려 있는지
알려면 문서 네 개를 역방향으로 diff해야 했다.

그래서 **현재 상태는 `docs/STATUS.md` 한 곳에서만 읽고 쓴다.** 버전 문서는 배경·변경·
검증까지만 담는다. v1.11.17 이전 문서에 남아 있는 `남은 과제` 섹션은 시점 기록이며
현재 상태가 아니다 — 고치지 말고 그대로 둔다.

되돌릴 수 있는 결정이나 나중에 판단이 갈릴 지점은 `STATUS.md` E절에 남긴다. 묻어
두면 다음 사람이 같은 논쟁을 반복한다.
