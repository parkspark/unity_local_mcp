...
- ver 1.11.29 - **접촉이 성립하지 않은 채 만들어지는 것을 검사로 만들었습니다.**
          A0의 표본이 안 쌓이는 이유 둘 중 측정 쪽은 v1.11.28이 닫았고, 남은 빌더
          쪽 둘을 여기서 처리했습니다 — 트리거를 안 만들거나(A4), 트리거에
          Rigidbody를 붙여 아이템이 월드 밖으로 떨어지거나(A5). 먼저 **브리지가
          콜라이더의 `isTrigger`를 안 내주고 있어** 호스트가 판별 자체를 못 했는데,
          `JsonUtility`가 엔진 콜라이더에서 아무 필드도 안 주는 탓이었습니다.
          Rigidbody와 같은 방식으로 노출했습니다. 검사는 **요청 어휘가 아니라 빌더가
          실제로 쓴 코드**를 근거로 하고, 관측된 참여자 전원이 트리거가 아니라고
          확인된 경우에만 실패로 삼습니다 — `OnCollisionEnter` 구현이나 판별 불가는
          걸리지 않습니다. 낙하 쪽은 원인이 아니라 결과를 재서 **기록된 189개 실행
          중 3개만 걸렸고 셋 다 실제 낙하, 오탐 0**이었습니다. 라이브 5회에서
          `disappeared: true`가 **4/5**(이전 10회에서는 0), 그중 한 번은 **새 검사가
          결함을 잡고 repair가 트리거를 설정해 획득이 실제로 일어났습니다.** 그리고
          A0의 표본이 처음으로 정렬됐습니다 — **표면 간격의 부호가 소멸과 일치**하고
          설명되지 않는 표본이 하나도 남지 않았습니다.
          상세: [docs/v1.11.29_contact_wiring_checks.md](docs/v1.11.29_contact_wiring_checks.md)

- ver 1.11.30 - **접촉 검사 둘을 결함 주입으로 확인했고, 조용히 꺼지던 경로를
          막았습니다.** v1.11.29의 두 검사는 단위 테스트로만 확인된 상태였는데,
          그것을 닫으러 갔더니 트리거 검사가 **`--repair-existing`에서 아예 돌지
          않고** 있었습니다 — `session_scripts`만 보는데 빌더 단계를 건너뛰면 그
          집합이 비기 때문입니다. 기존 씬을 재검증하는 경로 전체에서 검사가 없는
          것과 같았습니다. 이제 붙어 있는 컴포넌트 이름으로 `Assets/Scripts/<Name>.cs`도
          함께 읽습니다. 그 위에서 정상 씬에 결함을 하나씩 주입해 확인했습니다 —
          트리거를 끄니 `trigger_handler_without_trigger_collider`가, Rigidbody를
          non-kinematic으로 되돌리니 `contact_object_fell_away`가 떴고(아이템이
          y −36까지 낙하), **둘 다 repair가 고쳐 재검증에서 아이템이 실제로
          사라졌습니다.** 그리고 낙하 검사가 라이브에서 안 터지던 이유도 확인했습니다
          — 빌더가 안내대로 `isKinematic = 1`을 쓰고 있어 **결함이 생기지 않고**
          있었습니다.
          상세: [docs/v1.11.30_contact_checks_confirmed.md](docs/v1.11.30_contact_checks_confirmed.md)

- ver 1.11.31 - **캡슐을 절반 크기로 재고 있었고, 경계는 0이 아니었습니다.** A0을
          기록에서 검사로 승격하려면 임계값이 필요하고, 그러려면 뒤집히는 지점을
          찾아야 합니다. 통제된 스윕(아이템 높이만 변경)을 돌렸더니 첫 점에서
          말이 안 되는 값이 나왔습니다 — 아이템을 **올렸는데 간격이 더 음수**가
          됐습니다. 플레이어는 캡슐인데 **Unity 캡슐은 `localScale` 1에서 높이가
          2**라, `scale/2`로 잡은 세로 반폭이 실제의 절반이었습니다. v1.11.27 이후
          기록된 모든 접촉 간격에 그 오차가 실려 있었습니다. 브리지가 콜라이더의
          **월드 AABB(`bounds.center`·`bounds.extents`)**를 그대로 내주게 해서
          추정을 없앴습니다. 그 위에서 다시 쓸어 담은 결과가 이렇습니다 —
          **간격 −0.199·−0.349는 획득 안 됨, −0.499는 획득됨.** 경계가 0이 아니라
          **−0.35와 −0.50 사이**입니다. 부호만 보고 검사를 만들었다면 정상 씬이
          실패 판정을 받았을 것입니다. **여유가 왜 필요한지 아직 설명하지 못해
          승격은 보류**했습니다 — "설명하지 못한 것을 실패로 판정하지 않는다".
          상세: [docs/v1.11.31_collider_bounds_and_boundary_sweep.md](docs/v1.11.31_collider_bounds_and_boundary_sweep.md)

- ver 1.11.32 - **여유의 정체는 구르는 플레이어였습니다.** v1.11.31이 경계를 찾고도
          승격을 보류한 이유가 "여유가 왜 필요한지 설명 못 함"이었는데, 답이 실행
          로그에 있었습니다 — 플레이어 캡슐의 `Rigidbody.constraints = 0`이라
          **이동 중 Z축으로 굴러갑니다**(euler Z가 173°까지). 캡슐이 누우면 세로
          반폭이 1.0에서 0.5로 줄고, 호스트는 구간 **양 끝에서만** 형상을 재기 때문에
          그 사이의 기울기를 못 봅니다. 같은 씬에서 **회전만 잠근 A/B**가 결정적이었습니다
          — 간격 −0.199에서 회전 자유는 획득 실패, −0.198에서 회전 잠금은 획득 성공.
          **경계는 원래 0 근처였고 그 여유는 허수였습니다.** 기록된 150개 실행을 세어
          보니 **3분의 1(32%)이 완전히 넘어갑니다.** 그래서 간격 옆에
          `player_max_tilt`를 함께 남겨 어느 표본을 믿어도 되는지 가를 수 있게
          했습니다 — 구르는 것을 실패로 판정하지는 않습니다(그 때문에 실패한 실행이
          아직 없습니다).
          상세: [docs/v1.11.32_rolling_player_explains_the_margin.md](docs/v1.11.32_rolling_player_explains_the_margin.md)

- ver 1.11.33 - **A0을 기록에서 판정으로 올렸습니다.** v1.11.23부터 여섯 사이클 동안
          접촉 후보는 기록만 하고 있었습니다. 회전을 잠근 통제 스윕으로 경계를
          확정했습니다 — **간격 +0.100 안 사라짐 / −0.050 사라짐 / −0.198 사라짐**,
          즉 **경계는 0**입니다. `contact_target_did_not_react`가 뚫고 지나갔는데
          반응이 없으면 실패로 만듭니다. 발동 조건은 좁게 잡았습니다: "닿으면
          사라진다" 형태만(밟기는 호스트가 못 만들고, 클리어·체력은 소멸이 옳은
          신호가 아닙니다), 간격 −0.25보다 안쪽만, **기울기 15도 이하일 때만**
          (기운 실행은 `measured_checks`에서도 빼서 "측정 안 한 것을 측정했다"고
          적지 않습니다). 주입으로 **검출 → repair → 소멸 측정까지** 닫았습니다.
          양성 사례를 만드는 데 두 번 실패했고 둘 다 정보를 남겼습니다 — 트리거를
          끄면 아이템이 **벽이 되어** 관통 자체가 안 일어나고(그건 다른 검사의
          몫), **비활성 MonoBehaviour도 `OnTriggerEnter`를 받았습니다**(제 가정이
          틀렸습니다). **오탐 근거가 얇다는 것**은 문서와 STATUS에 명시했습니다 —
          과거 영수증 31건 중 29건은 기울기 필드가 없어 판정이 보류됩니다.
          상세: [docs/v1.11.33_contact_vanish_promoted.md](docs/v1.11.33_contact_vanish_promoted.md)

- ver 1.11.34 - **새 검사의 오탐 기준선을 실측했습니다(코드 변경 없음).** v1.11.33이
          "오탐 근거가 얇다"고 명시한 공백만 메웠습니다. 과거 로그로는 잴 수
          없습니다 — 간격·기울기가 새 필드라 복원하면 **캡슐을 다시 절반으로 재게
          되어 오탐을 과소평가**합니다. 그래서 요청 형태 둘(아이템 하나 / 코인 3개
          + 점수)로 라이브 4회를 돌렸습니다. **판정 3회·보류 1회, 오탐 0.** 함께
          드러난 것 셋: **적용률이 3분의 2**입니다(기울기 15° 초과면 판정 보류,
          기록 전체의 회전 분포와 일치) · `Coin_1`이 간격 −0.249로 **문턱(−0.25)에서
          0.001 차이**로 판정 밖이었습니다 · 코인 3개 중 **1개만 획득**됐고 나머지는
...
- ver 1.12.2 - **재시도가 진척을 알게 하니 계획이 5/5로 끝났습니다.** 마일스톤당
          12 + 재시도 12를 태우면 5개 계획에 예산 60은 절반뿐입니다. **한도를 올리는
          대신 낭비를 없앴습니다.** 재시도는 히스토리를 새로 시작하는데 **이미 만든
          파일이 무엇인지 몰라** 같은 것을 다시 만들고 있었습니다 — 이제 호스트가
          디스크에서 판별해 "이미 있는 파일 / 아직 없는 파일"로 나눠 싣고, 전부
          있으면 "남은 것은 씬 배치·컴포넌트 부착·검증이다"로 바꿔 말합니다. 그리고
          한도를 쳐도 산출물이 완비면 재시도를 건너뜁니다. 결과: 계획 모드 진행이
          **완료 2 → 1 → 3 → 5**로 움직여 **마일스톤 5개 전부 완료·재시도 0회·남은
          예산 5**가 나왔습니다. 다만 **판정은 여전히 `failed`**입니다 — 계획이
          끝까지 갔을 뿐 게임은 동작하지 않고, 미측정 수도 나아지지 않았습니다
          (9 → 9). 이 사이클이 산 것은 **완주**이지 측정 범위가 아닙니다. 자동 완료
...
- ver 1.12.5 - **재현성을 재니 `verified`는 1/6이었고, 원인은 벽이었습니다.**
          v1.12.4가 통과한 프롬프트를 씬 이름만 바꿔 5회 더 돌렸더니 **0/5**였습니다.
          실패한 회차의 기하를 겹쳐 보니 원인이 하나였습니다 — 빌더가 "시작 오른쪽
          8유닛은 평지"를 어기고 x=5에 **플레이어 머리보다 낮은** 플랫폼을 놓아
          오른쪽 이동이 x=2.5에서 정확히 막혔습니다. 막힌 방향에서는 부스트가 빨라져도
          거리가 같으므로 증상은 `boost_distance_too_short`로 나오고 **repair는 멀쩡한
          부스트 코드를 고치러 갑니다**(한 회차는 같은 실패로 두 사이클 소모).
          배치 문제라고 정확히 말해 주는 `movement_path_blocked`는 이미 있었지만 d/a
          비율 0.5 문턱에서 갈렸습니다 — **같은 원인의 비율이 0.492 / 0.502 / 0.570**로
          나와 1/3만 이름을 받았습니다. 문턱을 내리는 대신 비율이 못 가르는 구간
          (0.5~0.85)만 **멈춘 자리의 기하**로 판정하게 했습니다: 멈춘 표면과 벽면이
          맞닿아 있고 세로로 실제 겹칠 때만. 밟고 선 바닥은 겹치지 않아 빠집니다.
          찾은 벽은 이름으로 나가고(`blocking_objects`), 벽이 있으면 부스트 안내에서
          코드 수정 항목을 뺍니다. 결과: 같은 조건 5회 재측정에서 **0/5 → 1/5**,
          게이트가 발동한 3회는 **전부 다음 사이클에 배치가 고쳐졌고**(3/3) 오탐 0,
          측정 완결성 3/5 → 5/5. 다만 10회 중 `verified`는 2회로 재현성은 여전히
          낮고, 이 측정이 **부스트 문턱 1.4가 요청의 0.18초·2배와 어긋난다**는 것을
          새로 드러냈습니다(상한이 1.36).
          상세: [docs/v1.12.5_wall_named.md](docs/v1.12.5_wall_named.md)

- ver 1.12.6 - **요청을 완벽히 지킨 구현이 통과할 수 없었습니다.** v1.12.5가 벽을
          치우고 나니 남은 실패가 부스트 비율 1.39·1.40·1.39로 몰렸는데, 문턱이
          1.4였습니다. 요청은 **지속 0.18초·속도 2배**라고 쓰고 호스트는 **0.5초
          구간** 거리를 비교하므로, 요청대로 정확히 구현했을 때의 상한은
          `(0.18×2 + 0.32)/0.5 = 1.36`입니다 — **통과가 불가능했고**, 실제로 통과한
          회차는 지속시간이나 배수를 요청보다 크게 잡은 쪽이었습니다. 뿌리는 이름
          하나였습니다: `boost_duration`이 **측정 창**과 **요청의 지속시간** 둘 다로
          쓰였습니다. 이름을 갈라 `boost_measure_window`로 두고, 요청에서 지속시간과
          배수를 읽어 **기대 비율을 계산**한 뒤 그 85%를 문턱으로 삼습니다. 계수는
          영수증 55건의 분포에서 골랐습니다 — 명시 요청은 실패 ≤1.24 / 성공 ≥1.38,
          미명시 요청은 실패 ≤1.22 / 성공 ≥1.96으로 **두 빈 구간에 같은 계수가
          들어갑니다**(각각 문턱 1.31, 1.85 — 미명시 쪽은 오히려 **올라갑니다**).
          같은 요청에 적힌 합격 조건 "1.4배 이상"을 배수로 오독하면 문턱이 자기
          자신을 재므로 속도 문맥만 읽습니다. 결과: 기록된 표본 재판정에서 **새로
          통과 4건·역전 0건**, 라이브 5회에서 **`verified` 2/5**이고 통과한 두 회차의
          비율이 **둘 다 1.394**입니다(이전이라면 둘 다 실패). 부스트가 실제로 망가진
