...
- **"The open scene(s) have been modified externally" 프롬프트** — `SceneAutoReload.cs`
  (브리지 옆 Editor 스크립트)가 자동으로 씬을 다시 열어 이 프롬프트를 없앤다.
  단, 에디터에서 **저장 안 한 변경이 있는 씬(dirty)**은 작업 보호를 위해 자동 리로드하지 않고
  Unity 기본 프롬프트를 그대로 둔다. 끄려면 Tools ▸ MCP Bridge ▸ Auto-Reload Scenes On
  External Change 체크 해제.
...
| `local_tools.py` | 호스트 로컬 도구 (unity_write_script / unity_read_script) |
