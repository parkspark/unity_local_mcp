...
### 로컬 도구 (C# 스크립트 작성 및 삭제)

MCP 서버의 18개 도구 외에, 호스트가 직접 실행하는 도구 3개가 추가됩니다:
...
| `local_tools.py` | 호스트 로컬 도구 (unity_write_script / unity_read_script / unity_delete_script), 구식 API 자동 교정 |
| `agent.py` | 시스템 프롬프트 + tool-call 루프, 누수 및 혼합 응답 tool-call 복구 파서 |
